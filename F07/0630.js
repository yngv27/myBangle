...
** BEGIN WATCH FACE
*/
const startX=[6,43,6,43],startY=[14,14,82,82],nmX=[4,42,88,126],nmY=[12,12,12,12];
let xS=.4,yS=.6,rotate=!1,w=60,h=90,fillDigits=true,fgColor;
...
function ellipse(x1, y1, x2, y2, fill) {
  if (fill) g.fillEllipse(x1*xS, y1*yS, x2*xS, y2*yS);
  else g.drawEllipse(x1*xS, y1*yS, x2*xS, y2*yS);
}
...
function rect(x1, y1, x2, y2, fill) {
  if (fill) g.fillRect(x1*xS, y1*yS, x2*xS, y2*yS);
  else g.drawRect(x1*xS, y1*yS, x2*xS, y2*yS);
}
...
function drawDigit(pos, dig, nm) {
  let x=(nm?nmX:startX)[pos],y=(nm?nmY:startY)[pos];EMULATOR&&(x+=80);
