...
function init(){
  ...
  cmd(0x11); // sleep out
  delayms(120);
  // 0x0 for Magic3; 0x48 for QY03 (reverse L/R, reverse RGB)
  cmd([0x36, 0x48]);     // MADCTL - This is an unrotated screen
  cmd([0x37, 1, 64]); //256+44=300 = offset by -20 so no need to add +20 to y
  ...
  cmd([0xB2, 0xb, 0xb, 0x33, 0x33, 0x33]); // PORCTRL (B2h): Porch Setting
  //cmd([0xB2, 0xC, 0xC, 0x0, 0x33, 0x33]);
  
  cmd([0xB7, 0x35]); //11]);     // GCTRL (B7h): Gate Control
  cmd([0xBB, 0x35]);  // VCOMS (BBh): VCOM Setting 
  cmd([0xC0, 0x2c]);
  cmd([0xC2, 1]);     // VDVVRHEN (C2h): VDV and VRH Command Enable
  cmd([0xC3, 8]);  // VRHS (C3h): VRH Set 
  cmd([0xC4, 0x0]);  // VDVS (C4h): VDV Set
  ...
  // use 0x21 for Magic3, do NOT for QY03
...
function drawClock(){
  ...
  g.setColor(13);
  for(let i=0; i<8; i++) {
    g.drawLine(0,i*10,239,i*10);
    g.drawLine(0,280+i*10, 120, 280+i*10);
  }
  //g.drawRect(16,16,223,263);
  g.setColor(14);
  g.drawRect(8,8,231,271);
  g.setColor(15);
  g.drawRect(0,0,239,279);
  g.setFont("8x16");
...
},D4,{ repeat:true, edge:'rising',debounce:25 }
);
