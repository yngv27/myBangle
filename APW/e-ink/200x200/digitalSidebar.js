...
function init() {
  ...
  let start = () => {
    g.setBgColor(_C.BG).setColor(_C.FG).fillCircle(78,100,4).fillCircle(78,132,4);
  ...
  let showMsg = (msgobj) => {
    g.setFont("Knx18",1).setFontAlign(0,0);
    clrDtWdw(_C.MSGBG); //0,0.2,0.15);
    //g.drawBold( msgobj.text.split('|').join("\n"), _C.XMID, 160);
  ...
  let drawClock = (t) => {
    ...
    for(let d = 0; d < digs.length; d++) {
      ...
      let x=-14;
      drawDigit(digs[d], [x,x+44,x+100,x+140][d], 64, 0.75, 0.75);
    }
    sidebar(t);

  };
  let drawData = (d) => {
    g.setFont("Default",1).setFontAlign(0,0).setBgColor(_C.BG).setColor(_C.FG);
  ...
  function sidebar(d) {
    ...
    for(let d = 0; d < digs.length; d++) {
      //drawDigit(digs[d], [167,181][d], 112, 0.25, 0.25);
    }
    */
    g.setFont("Default",2).setFontAlign(0,0);
    g.setRotation(1).setColor(_C.FG);
    g.fillRect(0,0,199,21).setColor(_C.BG);
    g.drawString(`${d.hr}:${('0'+d.min).slice(-2)} ${d.isPM?"PM":"AM"}`,100, 14);
    g.setRotation(0).setColor(_C.FG);
  }
  /*
  let stop = () => {
...
exports.start();
//*
let dt = { hr: 12, min: 35, niceDate: "Sun Jul 12", isPM: true };
