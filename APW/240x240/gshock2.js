...
let drawClock = function(dt) {
  ...
  for(let d = 0; d < digs.length; d++) {
    ...
    if(d < 2) g.setColor("#80E0E0"); else g.setColor(C.FGCOLOR);
    // drawDigit(digs[d], [50,72,101,123][d], 48); //unscaled
    drawDigit(digs[d], [x,x+32,x+76,x+106][d], 44); //unscaled
  }
  lastDigs = digs;
  tick();
};
...
function tick() {
  ...
  for(let d = 0; d < digs.length; d++) {
    if(digs[d] != ldigs[d])
      drawDigitMini(digs[d], [x,x+17][d], 67);
  }
  lastSecs = s;
}
logD=print;
exports.drawData = function (dt) {
  ...
  let b = E.getBattery();
  g.drawString(` ${b} % `, x+100, 20, true);
  g.drawString(` ${b} % `, x+101, 20,false);
  
  if(typeof(future) != "undefined")
    g.drawString(` ${tDiff(future)} `, 120, 154, true);
};
...
function drawAmPm(isPM) {
  ...
  let v = isPM ? "    \nPM " : "AM\n     ";
  let x = 216;
  g.clearRect(x, 59, 239, 99);
  g.drawString(v, x, 59, true);
  g.drawString(v, x+1, 59, false);
}
function drawDOW(dow) {
  let v = "SUNMONTUEWEDTHUFRISAT".substring(dow*3,(dow+1)*3);
  let x=216;
  g.clearRect(208, 124, 239, 144);
  g.setColor(C.FGCOLOR).drawString(v, x, 126, true);
  g.setColor(C.FGCOLOR).drawString(v, x+1, 126, false);
}

function tDiff(moment) {
  let dt = new Date();
  let s = 1;
  let m = Math.floor((dt.getTime()-moment)/60000);
  if(m < 0) { s=0; m*=-1;}
  let h = Math.floor(m / 60); m = m % 60;
  let dz = Math.floor(h / 24); h = h % 24;
  return(`${s?'+':'-'}${dz}d ${h}h ${m}m`); 
}
