...
function calendar() { //x0, y0, w, h) {
  ...
  buf.setFontTamzen(); //setFontVector(16); //("HaxorNarrow7x17",2);
...
function newday() {
  g.cls();
  //  msgline(motd, 272);
  msgline((new Date().toString().substring(0,15)), 272);
  agenda();
  todo();
  E.at("17:00", calendar);
  E.at("00:00", calendar);
...
function todoBkup() { _S.writeJSON("todos",[w.list,n.list]); }
...
function updTodo(parms) {
  ...
  g.update(128, i*30+16, g3);
  debug("I got "+i);
}
function todo() {
  let cnt=0;
  w.list.filter((x)=>{ return x.pri == "A"; }).forEach((t,i)=>{
    if(cnt < 9) ifFree({func:updTodo, parm:[cnt++, t.task]});
  });
...
function clrAgenda(i) { if(i>8) i-=9; _agenda[i]=''; ifFree({func:updAgenda, parm: i});}
//* Agency
...
//*/
//* Tamzen
Graphics.prototype.setFontTamzen = function(scale) 
...
let gM=Graphics.createArrayBuffer(400,24,1,{msb:true});
gM.setFontDefault();
...
function msgline(msg, y) {
  ifFree({func:function(p) {
    gM.clear();
    gM.drawString(p[0], 8, 3);
    gM.drawString(p[0], 9, 3);
    g.update(0, p[1], gM);
  }, parm: [bold(msg),y]});
}
