...
let UI = {
  ...
  println: function(str) { 
    g.drawString(str, 8, UI.cursor); UI.cursor += g.getFontHeight(); 
  },
  invert: function() { 
  ...
  drawBanner: () => {
    if(!UI.bannerMsg.length) {
      ...
      g.redbuf.drawString(UI.bannerMsg, 200, g.redbuf.getHeight()/2);
  ...
  offDuty: (str)=> {
    if(!str) str = "ZZzz...";
    UI.cls(); g.setFontRoboto20().setFontAlign(0,0);
